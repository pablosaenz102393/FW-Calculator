...
export default function MaturityStep() {
  ...
  const handleContinue = () => {
    ...
    try {
      ...
      // Helper function to get percentage for a component based on maturity level
      const getPercentageForComponent = (componentId: string): number => {
        const mv = maturityValues[componentId]
        const comp = visibleComponents.find(c => c.id === componentId)
        
        if (mv?.level === 'Other' && mv.customPercentage !== undefined && !isNaN(mv.customPercentage)) {
          return mv.customPercentage
        } else if (mv?.level === 'Other') {
          return 0
        } else if (comp?.maturityDefaults) {
          const defaults = comp.maturityDefaults
          switch (mv?.level) {
            case 'Low':
              return defaults.low?.percentEliminated || 0
            case 'Medium':
              return defaults.medium?.percentEliminated || 0
            case 'High':
              return defaults.high?.percentEliminated || 0
          }
        }
        return 0
      }

      // Calculate deflection percentages first (needed for productivity components)
      const kbDeflectionPercent = getPercentageForComponent('knowledgeBase')
      const asrDeflectionPercent = getPercentageForComponent('automationServiceRequests')

      // Calculate remaining tickets after deflection
      const totalIncidents = agentData.annualIncidents || 0
      const totalServiceRequests = agentData.annualServiceRequests || 0
      const incidentsRemaining = totalIncidents - (totalIncidents * (kbDeflectionPercent / 100))
      const serviceRequestsRemaining = totalServiceRequests - (totalServiceRequests * (asrDeflectionPercent / 100))
      ...
      visibleComponents.forEach((component) => {
        ...
        if (component.id === 'knowledgeBase') {
          // KB deflects incidents only (service requests are handled by automationServiceRequests)
          inputs.numberOfTickets = totalIncidents
          ...
          inputs.numberOfTickets = totalServiceRequests
          inputs.percentEliminated = percentage
          inputs.timeSavedMinutes = maturityValue?.level === 'Other' && maturityValue.customTimeSaved !== undefined
            ? maturityValue.customTimeSaved
        ...
        // These represent time saved on remaining tickets after elimination
          ...
          // Always enable - uses remaining incidents after KB ticket deflection
          shouldEnable = true
          inputs.ticketsRemaining = incidentsRemaining
          ...
          // Always enable - uses remaining service requests after ASR automation
          shouldEnable = true
          inputs.requestsRemaining = serviceRequestsRemaining
